...
export default function RuleFindings({ rules }: Props) {
  if (!rules?.length) return (
    <div className="text-sm text-slate-400 py-4 text-center">No rule violations detected</div>
  )
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs text-slate-400 font-mono">{r.rule_id}</span>
                <span className="text-xs text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded">{r.type}</span>
              </div>
              <p className="text-sm text-slate-700 mt-1 leading-snug">{r.description}</p>
            </div>
          </motion.div>
        )
      })}
