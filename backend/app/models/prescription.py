...
class Prescription(Base):
    ...
    input_type = Column(String(64), nullable=True)          # image/jpeg, application/pdf, text/plain
    ...
    extracted_fields = Column(JSONB, nullable=True)         # structured JSON from GPT-4o
    ocr_confidence = Column(Float, nullable=True)           # 0.0-1.0
    
    # 🔥 FIXED: split signals properly
    ocr_confidence = Column(Float, nullable=True)         # readability
    completeness_score = Column(Float, nullable=True)     # missing data
    missing_fields = Column(JSONB, nullable=True)         # list of missing fields
    
    # Validation output
    drug_validation_result = Column(JSONB, nullable=True)   # rxnorm / openfda findings
